...
@section('section_name')
  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="#">{{__('dashboard.ecommerce')}}</a></li>
                    ...
                    <div class="card-body">
                                ...
                                <ul class="dashboard-tabs nav nav-pills nav-danger row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
                                    ...
                                    <!--end::Item-->
                                       <!--begin::Item-->
                                       <li class="nav-item d-flex col-sm flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
                                        <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center   @if($type==7) active  @endif "  href="{{route('Admin.order.index',6)}}">
                                            <span class="nav-icon py-2 w-auto">
                                                <span class="svg-icon svg-icon-primary svg-icon-2x"><!--begin::Svg Icon | path:C:\wamp64\www\keenthemes\themes\metronic\theme\html\demo1\dist/../src/media/svg/icons\General\Shield-disabled.svg--><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                        <rect x="0" y="0" width="24" height="24"/>
...
<script>
  ...
  <script>
    ...
    var routeAcceptOrder    ={!!json_encode(route('Admin.order.accept',0))!!};
    var routeRejectOrder    ={!!json_encode(route('Admin.order.reject',0))!!};

    var routeDeliveryOrder    ={!!json_encode(route('Admin.order.delivery',0))!!};
    var routeCompeleteOrder    ={!!json_encode(route('Admin.order.compelet',0))!!};
    ...
    $(document).ready(function() {
        $('#datatable').DataTable({
                "ajax": {
                    "url": `/list/order/${type}`, // Replace with your API endpoint
                    "type": "GET",
                },
                "columns": [
                    ...
                    { "data": "payment_type",   render: function (data, type, row, meta) {
                        return ` ${row.payment_type==1 ? `{{__('dashboard.wallet')}}` : ``}
                                 ${row.payment_type==2 ? `{{__('dashboard.bank')}}` : ``}
                                 ${row.payment_type==3 ? `{{__('dashboard.visa')}}` : ``}
                    ...
                    { "data": "created_at" },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            if(row.type==1)
                            return`
                                    <a href="${routeAcceptOrder}${row.id}">{{__('dashboard.accept')}}</a>|
                                    <a href="${routeRejectOrder}${row.id}">{{__('dashboard.reject')}}</a>
                                ...
                                `;
                                    ...
                                    return `
                                    <a href="${routeDeliveryOrder}${row.id}">{{__('dashboard.delivery')}}</a>
                                    <span   class="show_order  svg-icon svg-icon-primary svg-icon-2x " data-id="${row.id}"  data-toggle="modal" data-target="#exampleModal"><!--begin::Svg Icon | path:C:\wamp64\www\keenthemes\themes\metronic\theme\html\demo1\dist/../src/media/svg/icons\General\Visible.svg--><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                        <rect x="0" y="0" width="24" height="24"/>
                                        <path d="M3,12 C3,12 5.45454545,6 12,6 C16.9090909,6 21,12 21,12 C21,12 16.9090909,18 12,18 C5.45454545,18 3,12 3,12 Z" fill="#000000" fill-rule="nonzero" opacity="0.3"/>
                                ...
                                `;
                                    ...
                                    return `
                                    <a href="${routeCompeleteOrder}${row.id}">{{__('dashboard.completed')}}</a>|
                                    <span   class="show_order  svg-icon svg-icon-primary svg-icon-2x " data-id="${row.id}"  data-toggle="modal" data-target="#exampleModal"><!--begin::Svg Icon | path:C:\wamp64\www\keenthemes\themes\metronic\theme\html\demo1\dist/../src/media/svg/icons\General\Visible.svg--><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                        <rect x="0" y="0" width="24" height="24"/>
                                        <path d="M3,12 C3,12 5.45454545,6 12,6 C16.9090909,6 21,12 21,12 C21,12 16.9090909,18 12,18 C5.45454545,18 3,12 3,12 Z" fill="#000000" fill-rule="nonzero" opacity="0.3"/>
      ...
                    },
                ...
                "lengthMenu": [25, 50, 100], // Number of records per page options
                "autoWidth": true,
                "language": {
