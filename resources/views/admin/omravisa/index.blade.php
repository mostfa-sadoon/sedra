...
<h2><a href="{{route('Admin.omravisas','pending')}}">{{__('dashboard.omra_visa')}}</a></h2>
...
<div class="d-flex flex-column-fluid">
    ...
    <div class="container">
        <!--begin::Row-->
            ...
            <div class="row">
                <div class="col-3">
                    <button class="btn btn-primary mb-1" data-toggle="modal" data-target="#changeprice"> {{__('dashboard.omra_visa')}}</button>
                </div>
                        ...
                        <div class="card-body">
                                    ...
                                    <ul class="dashboard-tabs nav nav-pills nav-danger row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
                                        ...
                                        <li class="nav-item d-flex col-sm flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
                                            <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center @if($type=='pending') active  @endif"  href="{{route('Admin.omravisas','pending')}}">
                                        ...
                                        <!--begin::Item-->
                                        <li class="nav-item d-flex col-sm flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
                                            <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center @if($type=='accepted')  active  @endif"  href="{{route('Admin.omravisas','accepted')}}">
                                        ...
                                        <!--begin::Item-->
                                        <li class="nav-item d-flex col-sm flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
                                            <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center @if($type=='refused') active  @endif"  href="{{route('Admin.omravisas','refused')}}">
                    ...
                    <div class="card card-custom">
                        <div class="card-header">
                            <div class="card-title">
                                ...
                                <h3 class="card-label">{{__('dashboard.omra_visa')}}</h3>
                            </div>
                            ...
                            <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{__('dashboard.passport_img')}}</th>
                                        <th>{{__('dashboard.personal_img')}}</th>
        ...
        <div class="modal fade outer-repeater" id="changeprice" tabindex="-1" role="dialog" aria-labelledby="changeprice" aria-hidden="true">
            <div class="modal-dialog modal-xl bd-example-modal-xl" role="document">
                    ...
                    <div class="modal-content">
                        ...
                        <form action="{{route('Admin.omravisa.changeprice')}}" method="post">
                            @csrf
                        <div class="modal-body" id="detailes">
    ...
    <!-- start reject omravisa modal -->
                <div class="modal fade" id="rejectomra" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <form action="{{route('Admin.omravisas.reject')}}" method="post">
                            @csrf
...
<script>
 ...
 <script>
   ...
   // var omravisaRejectUrl           ={!!json_encode(route('Admin.barcode.reject',0))!!}
    var omravisaAcceptUrl           ={!!json_encode(route('Admin.omravisas.accept',0))!!}
    ...
    $(document).ready(function() {
        $('#datatable').DataTable({
                "ajax": {
                    "url": `/list/omravisa/${barcodetype}`, // Replace with your API endpoint
                    "type": "GET",
                },
                "columns": [
                    ...
                    { "data": "id" },
                    { "data": "name" },
                    { "data": "passport_img" , render: function (data, type, row, meta) {
                        return `
                            <td data-field="OrderID" aria-label="51079-129" class="datatable-cell">
                                    <span style="width: 250px;"><div class="d-flex align-items-center">
                                            <div class="symbol symbol-40 symbol-light-primary flex-shrink-0">
                                                <img class="" src="${row.passport_img}" alt="photo">
                                            </div>
                                    </span>
                                    </td>
                    ...
                    { "data": "personal_img" , render (data, type, row, meta) {
                        return `
                        <td data-field="OrderID" aria-label="51079-129" class="datatable-cell">
                            <span style="width: 250px;"><div class="d-flex align-items-center">

                                    <div class="symbol symbol-40 symbol-light-primary flex-shrink-0">
                                        <img class="" src="${row.personal_img}" alt="photo">
                                    </div>
                            </span>
                            </td>
                            ...
                            if(barcodetype=='pending'){
                                return `
                                    <span class="svg-icon svg-icon-primary svg-icon-2x"><!--begin::Svg Icon | path:C:\wamp64\www\keenthemes\themes\metronic\theme\html\demo1\dist/../src/media/svg/icons\Code\Done-circle.svg--><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                        <a href="${omravisaAcceptUrl}${row.id}">
                                        <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                            <rect x="0" y="0" width="24" height="24"/>
                                    ...
                                    <a class="reject_omravisa" data-id="${row.id}" data-toggle="modal" data-target="#rejectomra">
...
{{-- END get datatable --}}
        ...
        <script>
                $('#datatable').on('click','.show_omravisas',function (){
                ...
                $('#datatable').on('click','.reject_omravisa',function (){
